...
| **4 — Native diff** ✅ | `edit` patches → real diff editor + apply as pending changes | `details.patch`, `vscode.diff`, `TextDocumentContentProvider`, `WorkspaceEdit`, `workspace.applyEdit` | **Done (2026-06-23).** Completed `edit` tool cards show *View Diff* / *Apply* buttons; *View Diff* opens VS Code's diff editor (before↔after served by a `wingman-diff:` `TextDocumentContentProvider`, read-only), *Apply* writes the change as a `WorkspaceEdit` (surfaces in Source Control). Hardened `DiffService`: `cwd` always derived from `workspace.workspaceFolders` (never webview input), workspace-boundary validation via `realpath`, TOCTOU re-validation before each I/O, symlink-swap rejection on new-file parents, multi-file / deletion rejection, 1 MB patch cap enforced inside the service. Inline error banner on failure. Covered by 45 host + 5 webview unit tests |
