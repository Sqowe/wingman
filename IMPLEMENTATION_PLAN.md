...
> Instruction file visibility feature complete (2026-07-02).
...
| Session list scope | The session tree **and** the `/resume` picker show **only** sessions whose recorded `cwd` is an open workspace folder; out-of-workspace sessions are **hidden entirely** (exact match — a subdirectory is a different project). pi runs as one child process pinned to the workspace cwd and `switch_session` only loads a transcript (it does **not** re-root the agent), so resuming a foreign-project session would load its history but then read/edit/run against the *current* workspace — an incoherent, silent mismatch. Scoping to the open folder(s) is the only coherently resumable set. Pure helper `filterSessionsToCwds` (in `src/sessions/session-parse.ts`); tree/picker draw from `SessionTreeProvider.getScopedSessions()`. (2026-06-23) |
...
│   │   ├── pi-locator.ts     # resolve pi: configured path / PATH; version check
│   │   └── controller.ts     # owns session lifecycle, fans events to the webview
│   ├── commands/             # native command handlers (model, compact, new, fork…)
│   ├── diff/diff-service.ts  # patch → read-only vscode.diff preview (invertPatch)
...
| **4 — Native diff** ✅ | `edit` patches → real diff editor + apply as pending changes | `details.patch`, `vscode.diff`, `TextDocumentContentProvider`, `WorkspaceEdit`, `workspace.applyEdit` | **Done (2026-06-23).** Completed `edit` tool cards show *View Diff* / *Apply* buttons; *View Diff* opens VS Code's diff editor (before↔after served by a `wingman-diff:` `TextDocumentContentProvider`, read-only), *Apply* writes the change as a `WorkspaceEdit` (surfaces in Source Control). Hardened `DiffService`: `cwd` always derived from `workspace.workspaceFolders` (never webview input), workspace-boundary validation via `realpath`, TOCTOU re-validation before each I/O, symlink-swap rejection on new-file parents, multi-file / deletion rejection, 1 MB patch cap enforced inside the service. Inline error banner on failure. Covered by 45 host + 5 webview unit tests. **Revised 2026-07-09:** *Apply* / `WorkspaceEdit` removed (pi already writes the file; re-applying double-applied the patch). *View Diff* now reconstructs the "before" via `invertPatch` against the on-disk "after"; a `sqoweWingman.showViewDiffButton` boolean gates the button. |
| **5 — Commands** ✅ | User `/` menu + built-ins as native UI | `get_commands`; `contributes.commands/keybindings/menus`; `set_model` / `cycle_model` / `compact` / `new_session` / `fork` / `export_html` / `set_thinking_level` / `get_session_stats` | **Done (2026-06-23).** `/` autocomplete in the composer (fed by `get_commands`, inert built-in TUI commands filtered out, keyboard + click select). Native commands wired to RPC: Set/Cycle Model (`set_model` / `cycle_model` / `get_available_models`), Set/Cycle Thinking Level (`set_thinking_level` / `cycle_thinking_level`), Compact (`compact`, modal-confirmed), New/Fork/Clone Session (`new_session` / `fork` / `clone`), Export HTML (`export_html`), Show Stats — exposed in the command palette, view title bar, and session menu group. Always-visible status bar item (tokens / cost / messages) refreshed after each turn via `get_session_stats` (sequence-guarded), reset on new session. New Session clears the webview transcript via a `sessionReset` message; fork/clone preserve history. `Cmd/Ctrl+Alt+N` forwarded from the webview so the shortcut fires while the composer iframe has focus. Hardened: export-html path validation (realpath / extension / workspace-boundary), defensive `get_available_models` parsing, coalesced `get_commands` fetches. Covered by 126 host + 27 webview unit tests |
| **6 — UI protocol** ✅ | Map pi extension dialogs to native prompts | `extension_ui_request` → `showQuickPick` / `showInputBox` / `showWarningMessage`; `notify` → notifications | **Done (2026-06-23).** `UiProtocolBridge` maps pi's extension-UI sub-protocol to native VS Code UI: blocking dialogs (`select` → `showQuickPick`, `confirm` → modal `showWarningMessage`, `input` → `showInputBox`, `editor` → temp untitled doc + Submit/Cancel quick-pick) always reply with a matching `extension_ui_response` (value / confirmed / cancelled) — including on dismiss, handler error, unknown method, and dispose — so pi never deadlocks. Fire-and-forget methods map to native surfaces: `notify` → info/warning/error notifications; `setStatus` → webview status strip; `setWidget` → collapsible above/below-composer blocks; `setTitle` → header subtitle; `set_editor_text` → composer prefill. The bridge consumes **all** `extension_ui_request` events (incl. malformed) so none leak to the webview, and adds client-side timeout suppression so a late user response after pi's agent-side auto-resolve can't double-respond. New `AgentTransport.sendRaw()` writes responses without request correlation; provider buffers UI state until webview-ready and clears it on session reset. Protocol shapes verified against pi `0.79.10` `docs/rpc.md`. Covered by 31 host + 12 webview unit tests (157 host + 39 webview total) |
| **7 — Sessions** ✅ | List / switch / new / fork; reconnect | `switch_session`, `get_messages`, `get_fork_messages`; tree view; `workspaceState` | **Done (2026-06-24).** A **Sessions** tree view (activity bar) lists pi sessions read from `~/.pi/agent/sessions/`, **scoped to the open workspace folder(s)** (`filterSessionsToCwds` — pi's process cwd is pinned to the workspace, so other-project sessions can't be coherently resumed here), grouped by project with the current folder surfaced first/expanded and sessions newest-first. Switching calls `switch_session` + `get_messages` and replaces the webview transcript; the last session is restored on window reload via `workspaceState`. **Restored transcripts render with full fidelity** — the `get_messages` `AgentMessage` stream is reconstructed into the same chat items as the live event stream (assistant `toolCall` blocks paired with their `toolResult` → tool cards, `bashExecution` cards, thinking/text). The tree **auto-refreshes**: `AgentController.onSessionsChanged` fires on new/fork/clone, and a debounced `FileSystemWatcher` on the sessions dir catches sessions the pi CLI creates; a Refresh button sits in the view title bar. Parsing + grouping/scoping live in a pure, vscode-free `session-parse.ts`; message counting uses a `startsWith('{"type":"message"')` check (type is the first key) so message text can't inflate the count. Covered by +40 host + 17 webview unit tests (197 host + 56 webview total) |
| **8 — Config / trust** ✅ | cwd = workspace folder; project-trust flow; multi-root | `workspace.workspaceFolders`; pi project trust (`--approve` / `--no-approve`, `~/.pi/agent/trust.json`) | **Done (2026-06-24).** Native **project-trust gate**: on activation, `evaluateTrust` checks for trust-gated `.pi/` resources (`settings.json`, `extensions` / `skills` / `prompts` / `themes`, `SYSTEM.md` / `APPEND_SYSTEM.md`, and `.agents/skills` in the cwd or any ancestor) and reads the shared `~/.pi/agent/trust.json` (canonical-path map; the closest saved decision on the current/parent path wins). A saved decision is applied directly; otherwise a native modal (Trust / Don't Trust) asks once and persists the choice. The decision is passed to `pi --mode rpc` as `--approve` / `--no-approve` — RPC mode shows no trust prompt of its own, and the flag overrides for the run (verified against pi's CLI `--help` and `docs/security.md`). The pure, vscode-free `project-trust.ts` uses canonical/symlink-safe keys, backs up a malformed `trust.json` before overwriting, and distinguishes dismiss from explicit deny. **Multi-root**: `sqoweWingman.selectFolder` (view-title quick-pick, shown when `workspaceFolderCount > 1`) switches the active folder, re-runs the trust gate, persists the choice to `workspaceState`, and restarts pi pinned to the new cwd; `sqoweWingman.trustProject` re-prompts on demand. A persistent `onDidChangeWorkspaceFolders` watcher auto-switches + restarts (or tears down and waits) when the active folder is removed. All start/restart paths are serialized through `_serializedStart` so concurrent restarts never spawn two pi processes. Covered by +69 host unit tests (266 host + 56 webview total). |
| **Instruction file visibility** ✅ | Show which AGENTS.md/CLAUDE.md/SYSTEM.md/APPEND_SYSTEM.md files pi loaded in the status banner | Bundled pi extension (`-e`); `ctx.getSystemPromptOptions()`; reserved `setStatus` key; `InstructionFilesPopover` | **Done (2026-07-02).** A bundled pi extension (`pi-extensions/instruction-report/index.js`) registers `/wingman-instruction-report` and reports resolved context files + system-prompt overrides via `ctx.ui.setStatus('wingman:instructionFiles', JSON)`. `UiProtocolBridge._handleSetStatus` intercepts the reserved key and routes it to a constructor-supplied callback (never the generic status strip). `AgentController._reportInstructionFiles()` fires after each session (re)start: checks `get_commands` for the internal command, sends it silently, awaits the bridge callback with a 3s timeout, fires `onInstructionFiles`, and calls `provider.postInstructionFiles()`. The banner collapses to one line (`pi X.Y.Z ready · N instructions`) with a click-to-open `InstructionFilesPopover` (absolutely positioned, no document-flow impact). `null` (unavailable) renders distinctly from `files: []` (confirmed zero). Graceful degradation: absent command → immediate null; old pi lacking `getSystemPromptOptions()` → null; timeout → null + output-channel log. Internal command excluded from `/` autocomplete. Covered by +13 host unit tests (389 host + 80 webview total). |
...
unified diff. Render as a read-only **preview**: register a `TextDocumentContentProvider`
exposing before/after as virtual URIs, then
`vscode.commands.executeCommand('vscode.diff', before, after, 'edit: file.ts')`. VS Code's
diff editor provides syntax highlighting, inline/side-by-side toggle, and native copy for free.

> **Revised (2026-07-09):** pi's `edit` tool has *already written the file to disk* by the
> time we receive `details.patch` — the patch is a record of `baseContent → newContent`, not
> pending work. So the on-disk file is the "after", and we reconstruct the "before" by
> inverting the patch (`invertPatch`) and applying it to the current file. The original
> `WorkspaceEdit`/`workspace.applyEdit` **Apply** action was removed (it double-applied an
> already-applied patch and failed with a deletion/context mismatch). Visibility of the single
> *View Diff* button is controlled by the `sqoweWingman.showViewDiffButton` setting.
...
`commands.executeCommand('vscode.diff', …)`, `TextDocumentContentProvider`,
`window.{createStatusBarItem,showQuickPick,showInputBox}`,
